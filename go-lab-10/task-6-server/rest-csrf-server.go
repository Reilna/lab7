...
func generateCSRFToken() string {
	return "CSRF-Token" // В реальной реализации следует генерировать уникальный токен
}
	...
	r.HandleFunc("/login", func(w http.ResponseWriter, r *http.Request) {
		...
		for _, user := range users {
			if user.Name == u.Name {
				...
				mu.Lock()
				csrfTokens[user.Name] = csrfToken
				mu.Unlock()
				w.Header().Set("Content-Type", "application/json")
				json.NewEncoder(w).Encode(map[string]string{"token": token, "csrf_token": csrfToken})
				return
			}
		}
